...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                block.time = parseInt(self._getCurrentTimestamp());
                block.previousBlockHash = self.height < 0 ? null : self._getPreviousBlock().hash;
    ...
    _getElapsedMinutes(message) {
        const SEC_PER_MINUT = 60;
        let timestamp = parseInt(message.split(':')[1]);
        let currentTime = parseInt(this._getCurrentTimestamp());

        let result = ((currentTime - timestamp) / SEC_PER_MINUT);
        return result;
    }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            let block = self.chain.filter(value => value.hash === hash)[0];
